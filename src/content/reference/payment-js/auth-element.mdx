...
<Element type="auth" theme="light" hash="adedc7a6-3a65-453a-a1a2-0f83a3e8da1a" publicSecret="mp_e4c4131291c24ea3922c9f376367a4f1" environment="cdn.m.todaq.net" client:load />
<script>{`
    (() => {
        document.addEventListener("authorized", async (event) => {
            const { session: { session } } = event.detail;
            ...
            try {
                const response = await fetch('https://pay.m.todaq.net/v4/paywall', {
