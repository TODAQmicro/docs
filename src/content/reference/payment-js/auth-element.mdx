...
<Element type="auth" theme="light" hash="adedc7a6-3a65-453a-a1a2-0f83a3e8da1a" environment="cdn.stage.m.todaq.net" client:load />
<script>{`
    (() => {
        document.addEventListener("authorized", async (event) => {
            ...
            try {
                const response = await fetch('https://pay.stage.m.todaq.net/v3/paywall', {
                    method: 'POST',
                    headers: {
