...
```bash
curl -X GET "https://pay.m.todaq.net/v4/dq"
...
```bash
curl -X POST "https://pay.m.todaq.net/v4/twin/" \
...
```bash
curl -X POST "https://pay.m.todaq.net/v4/commodity/" \
...
- **Payment Flow**: Customer buys commodity → Payment goes to the owning Twin's balance

For more details, see the [Twins API reference](/reference/payment-api/v4/twins) and [Commodities API reference](/reference/payment-api/v4/commodity).
